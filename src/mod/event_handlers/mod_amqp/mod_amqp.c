...
// Bind to different XML sections as needed
static switch_xml_section_t sections[] = {
...
SWITCH_STANDARD_API(amqp_reload) { return mod_amqp_do_config(SWITCH_TRUE); }
...
SWITCH_MODULE_LOAD_FUNCTION(mod_amqp_load)
{
	...
	switch_xml_binding_t **binding = &mod_amqp_globals.directory_fetch_binding;
	switch_hash_index_t *hi = NULL;
	mod_amqp_xml_fetch_profile_t *directory_profile = NULL;
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	switch_core_hash_init(&(mod_amqp_globals.directory_hash));

	mod_amqp_set_hostname();

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_amqp loading: Version %s\n", switch_version_full());

	if (mod_amqp_do_config(SWITCH_FALSE) != SWITCH_STATUS_SUCCESS) { return SWITCH_STATUS_GENERR; }

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Searching mod_amqp profile hash\n");
	for (hi = switch_core_hash_first_iter(mod_amqp_globals.directory_hash, hi); hi; hi = switch_core_hash_next(&hi)) {
		switch_core_hash_this(hi, NULL, NULL, (void **)&directory_profile);
	}
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "FOUND mod_amqp profile hash\n");

	for (int i = 0; i < sizeof(sections) / sizeof(sections[0]); i++) {
		if (switch_xml_bind_search_function_ret(mod_amqp_fetch_xml_section, sections[i], directory_profile, binding) !=
			SWITCH_STATUS_SUCCESS) {
			switch_core_destroy_memory_pool(&pool);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Could not bind to FreeSWITCH %s XML requests\n",
							  xml_section_to_string(sections[i]));
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_amqp_shutdown)
{
	...
	mod_amqp_xml_fetch_profile_t *directory_profile;
	...
	while ((hi = switch_core_hash_first_iter(mod_amqp_globals.directory_hash, hi))) {
		switch_core_hash_this(hi, NULL, NULL, (void **)&directory_profile);
		mod_amqp_fetch_xml_destroy(&directory_profile);
	}
	...
	switch_core_hash_destroy(&(mod_amqp_globals.directory_hash));
//	switch_core_hash_destroy(&(mod_amqp_globals.xml_fetch_hash));
